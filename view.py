...
def snp():
	arg_dict = {
		'vcf': args_.vcf,
		'target_gene': args_.target_gene,
		'pair_list': args_.pair_list
	}

	helper.check_args(arg_dict)
...
def peek():
	...
	finalized_vcf = helper.read_vcf_minimum(args_.vcf)
	...
	for i in range(len(finalized_vcf.data)):
		...
		if 'D' in ps:		
			if len(alt) > 1:
				for j in range(n):
					if j + 1 > len(alt):
						finalized_vcf.data[i].append('0|1')
					else:
						finalized_vcf.data[i].append(f'0|{j + 1}')
			else:
				for j in range(n):
					finalized_vcf.data[i].append('0|1')
	...
	for name in args_.samples:
		snp_list += args_.samples[name].hap[0].obs
		snp_list += args_.samples[name].hap[1].obs
	...
	# get candidates
	cand_list = [v for k, v in args_.star_dict.items() if set(v.core).issubset(snp_list) and not v.sv]
	
	# write results
	with open(args_.extended_prefix + '.txt', 'w') as f:	
		header = ['name', 'score', 'core', 'tag', 'callable']
		f.write('\t'.join(header) + '\n')
...
SUBTOOLS = {'summary': summary, 'meta': meta, 'snp': snp, 'plotcov': plotcov, 'changepoint': changepoint, 'peek': peek}
...
available subtools:
  ...
  peek
    peek at 'finalized.vcf' and report all star alleles that can be called by the 'genotype' tool
...
main usages:
  ...
  run the peek sbutool
    stargazer.py view peek -o OUTPUT_PREFIX -t TARGET_GENE --vcf VCF
'''
