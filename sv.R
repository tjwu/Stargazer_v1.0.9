...
# read input GDF file
gdf <- read.table(gdf.nam, header = T, check.names = F, stringsAsFactors = F, comment.char = '', sep = '\t', nrows = 2)
cls <- c(c('character', 'numeric', 'numeric'), rep('integer', ncol(gdf) - 3))
gdf <- read.table(gdf.nam, header = T, check.names = F, stringsAsFactors = F, comment.char = '', sep = '\t', colClasses = cls)
