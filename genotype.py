...
def phase_vcf(conformed_vcf):
	...
	subprocess.call(['java', '-Xmx2g', '-jar', args_.program_dir + '/beagle.25Nov19.28d.jar', f'gt={args_.extended_prefix}.project/phaseme.vcf', 'chrom=' + args_.target_region, 'ref=' + args_.ref_vcf, f'out={args_.extended_prefix}.project/phased','burnin=4', 'iterations=4', 'phase-states=4', 'impute=' + str(args_.impute).lower()], stdout = open(f'{args_.extended_prefix}.project/phased.log', 'a'), stderr = open(f'{args_.extended_prefix}.project/phased.log', 'a'))
...
def run(args):
	...
	args_.control_gene = '.' if not args_.control_gene or args_.control_type != 'known' else args_.control_gene
	args_.sample_list = ','.join(args_.sample_list) if args_.sample_list else '.'
	
	helper.create_dir(f'{args_.extended_prefix}.project')
		
	log_genotype_mode() # step 1
...
# 	helper.write_vcf(processed_vcf, f'{args_.extended_prefix}.project/processed.vcf')
	adjusted_vcf = adjust_vcf(processed_vcf)
# 	helper.write_vcf(adjusted_vcf, f'{args_.extended_prefix}.project/adjusted.vcf')
	conformed_vcf = conform_vcf(adjusted_vcf, ref_vcf) # step 4
