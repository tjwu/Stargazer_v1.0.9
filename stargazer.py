...
def main():
	...
	setup_parser.add_argument('-t', '--target_gene', choices = helper.get_target_genes(), metavar = 'TARGET_GENE', type = str.lower, help = 'target gene name')
	setup_parser.add_argument('--vcf', type = helper.check_file, help = 'input VCF file (.vcf or .vcf.gz)')
	setup_parser.add_argument('--vcf_dir', type = helper.check_file, help = 'directory with VCF files')
	...
	pipeline_parser.add_argument('--assembly', type = helper.check_file, help = 'reference genome assembly file (.fa or .fasta)')
	pipeline_parser.add_argument('--bam', type = helper.check_file, help = 'input BAM file')
	pipeline_parser.add_argument('--bam_list', nargs = '*', metavar = 'BAM', type = helper.check_file, help = 'space-delimited list of BAM files')
	pipeline_parser.add_argument('--batch', type = int, default = 1, help = 'number of batches used for parallelization (default: 1)')	
	pipeline_parser.add_argument('--bed', type = helper.check_file, help = 'input BED file')
	pipeline_parser.add_argument('-c', '--control_gene', help = 'control gene name')
	...
	pipeline_parser.add_argument('--dbsnp', type = helper.check_file, help = 'dbSNP file (.vcf or .vcf.gz)')
	...
	pipeline_parser.add_argument('--variant_list', nargs = '*', metavar = 'VARIANT', type = helper.check_file, help = 'known variants used for BQSR')
	pipeline_parser.add_argument('--length', type = int, default = 150, help = 'read length used')
	...
	pipeline_parser.add_argument('--platform', default = 'illumina', help = 'sequencing platofrm used')
	...
	pipeline_parser.add_argument('--thread', type = int, default = 1, help = 'number of threads used for parallelization (default: 1)')	
